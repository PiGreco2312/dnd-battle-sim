...
def write_weapons(weapons, filename):
    with open(filename, 'w') as file:
        for weapon in weapons:
            file.write(f"{weapon['Name']}{VALUES_SEPARATOR}")
            file.write(f"{weapon['Category']}{VALUES_SEPARATOR}")
            file.write(f"{weapon['Range']}{VALUES_SEPARATOR}")
            file.write(f"{weapon['Damage']}{VALUES_SEPARATOR}")
            file.write(f"{weapon['Type']}{VALUES_SEPARATOR}")
            for prop in weapon['Properties']:
                if prop != weapon['Properties'][-1]:
                    file.write(f"{prop}{BETWEEN_VALUES_SEPARATOR}")
                else:
                    file.write(f"{prop}{VALUES_SEPARATOR}")
...
def load_monsters():
    ...
    if response.status_code == 200:
        ...
        for monster in data['results'][:10]:
            ...
            if monster_response.status_code == 200:
                ...
                monster_list.append({
                    ...
                    "Vulnerabilities": [v.capitalize() for v in monster['damage_vulnerabilities']] if 'damage_vulnerabilities' in monster and monster['damage_vulnerabilities'] else "No_Vulnerabilities",
                    "Resistances": [r.capitalize() for r in monster['damage_resistances']] if 'damage_resistances' in monster and monster['damage_resistances'] else "No_Resistances",
                    "Immunities": [i.capitalize() for i in monster['damage_immunities']] if 'damage_immunities' in monster and monster['damage_immunities'] else "No_Immunities",                 
                    ...
                    "Actions": [{
                        ...
                        "Attack_Bonus": action['attack_bonus'] if 'attack_bonus' in action else "No_AttackBonus",
                        "DC": action['dc']['dc_type']['name'] if 'dc' in action else "No_DC",
                        "Damage": action['damage'][0]['damage_dice'] if 'damage' in action else "No_Damage",
                        "Type": action['damage'][0]['damage_type']['name'] if 'damage' in action else "No_DamageType"
...
def write_monsters(monsters, filename):
    with open(filename, 'w') as file:
        for monster in monsters:
            file.write(f"{monster['Name']}{VALUES_SEPARATOR}")
            file.write(f"{monster['Size']}{VALUES_SEPARATOR}")
            file.write(f"{monster['Type']}{VALUES_SEPARATOR}")
            file.write(f"{monster['Alignment']}{VALUES_SEPARATOR}")
            file.write(f"{monster['AC']}{VALUES_SEPARATOR}")
            file.write(f"{monster['HP']}{VALUES_SEPARATOR}")
            file.write(f"{monster['Challenge Rating']}{VALUES_SEPARATOR}")

            speed = monster['Speed']
            for mode, value in speed.items():
                if mode != 'Swim':
                    file.write(f"{value}{BETWEEN_VALUES_SEPARATOR}")
                else:
                    file.write(f"{value}{VALUES_SEPARATOR}")

            damage_reduction_vector = ['Vulnerabilities', 'Resistances', 'Immunities']
            for attr in damage_reduction_vector:
                value = monster[attr]
                if isinstance(value, list):
                    joined_values = BETWEEN_VALUES_SEPARATOR.join(value)
                    file.write(f"{joined_values}{VALUES_SEPARATOR}")
                else:
                    file.write(f"{value}{VALUES_SEPARATOR}")
            ...
            if monster['Abilities'] != "No_Abilities":
                ...
                for i, ability in enumerate(monster['Abilities']):
                    if i!=lenght-1:
                        ...
                        file.write(f"{ability['Desc']}{BETWEEN_VALUES_SEPARATOR}")
                    else:
                        file.write(f"{ability['Name']}:")
                        file.write(f"{ability['Desc']}{VALUES_SEPARATOR}")
            else:
                file.write(f"{monster['Abilities']}{VALUES_SEPARATOR}")
                ...
                if monster[attr] is not ("No_BonusActions" or monster[attr] == "No_Reactions" or monster[attr] == "No_LegendaryActions"):
                    ...
                    for i, action in enumerate(monster[attr]):
                        if i!=lenght-1:
                            ...
                            file.write(f"{action['Desc']}{BETWEEN_VALUES_SEPARATOR}")
                        else:
                            file.write(f"{action['Name']}:")
                            file.write(f"{action['Desc']}{VALUES_SEPARATOR}")
                else:
                    file.write(f"{monster[attr]}{VALUES_SEPARATOR}")
            file.write("\n")
